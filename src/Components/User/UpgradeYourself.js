...
export default function UpgradeYourself({ posts }) {
  ...
  // Filter posts by category names that match categoryFilter
...
const HorizontalBlogCard = ({ post }) => (
  <Link
    ...
    <div className="p-4 flex flex-col items-start justify-start gap-2 w-full lg:w-2/3">
      <h2
        className="text-lg lg:text-xl font-semibold line-clamp-2"
...
const LatestBlogs = ({ post }) => (
  <Link
    ...
    <div className="p-4">
      ...
      />
      <p
        className="text-gray-600 mb-4 text-sm lg:text-base line-clamp-2"
        dangerouslySetInnerHTML={{ __html: post?.seoDescription }}
      />
    </div>
  </Link>
